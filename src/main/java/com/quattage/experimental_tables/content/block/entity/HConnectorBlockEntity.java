...
public class HConnectorBlockEntity extends BaseElectricTileEntity implements IWireNode, IObserveTileEntity, IHaveGoggleInformation {
    ...
    public HConnectorBlockEntity(BlockEntityType<?> tileEntityTypeIn, BlockPos pos, BlockState state) {
	...
	public Vec3d getNodeOffset(int node) {
		switch(getCachedState().get(HConnectorBlock.FACING)) {
			case DOWN:
				return OFFSET_DOWN;
			case UP:
				return OFFSET_UP;
	...
	public boolean isEnergyInput(Direction side) {
		return getCachedState().get(HConnectorBlock.FACING) == side;
	}
	...
	private void networkTick(EnergyNetwork en) {
		if(world.isClient())
			return;
		Direction d = getCachedState().get(HConnectorBlock.FACING);
