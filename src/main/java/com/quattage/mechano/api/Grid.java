...
// these words aren't in the bible
    ...
    public static @Nullable ServerGrid server(LevelReader world) {
        ...
        Grid attachment = sl.getData(MechanoData.POWER_GRID.get());
        Grid.weakServerGrid = new WorldlyReference<ServerGrid>(attachment.asServer());
        return Grid.weakServerGrid.get();
    }
    ...
    public static @Nullable ClientGrid client(LevelReader world) {
        ...
        Grid attachment = cl.getData(MechanoData.POWER_GRID.get());
        if(attachment == null) throw new IllegalStateException("Failed to acquire a client-sided dispatcher in" + world);
        Grid.weakClientGrid = new WorldlyReference<ClientGrid>(attachment.asClient());
        return Grid.weakClientGrid.get();
    }
