...
public class GriddableTerminus implements OrientationUpdatable, SourceProvider {
    ...
    public GriddableTerminus initializeFrom(CircuitComponent component) {
        ...
        if(component instanceof Node n) {
            Collection<AncillaryNode<?>> ancillaries = n.getAncillaries(); 
            this.ancillaries = ancillaries == null || ancillaries.isEmpty() ? null : ancillaries.toArray(new AncillaryNode[ancillaries.size()]);
            return this;
        }
        Mechano.LOGGER.warn("Skipped attempt to update ancillaries from an irrelevent source '" + component.getClass().getSimpleName() + "'");
        return this;
    }
    ...
    public void showAll(Vector3d basis) {
        forEach(ancillary -> { ancillary.drawToOutliner(basis, VectorOperations.toColor(basis), 1f, 1f); });
    }
    ...
    public void updateOrientation(CombinedOrientation dir) {
        forEach(ancillary -> {
            if(ancillary instanceof OrientationUpdatable ou)
                ou.updateOrientation(dir);
        });
    }

    /**
     * Iterates over all {@link AncillaryNode ancillaries}
     * in this holder. The input consumer won't be executed
     * at all unless this holder has been {@link #initializeFrom initialized}
     * onto a {@link CircuitComponent} with at least one {@link AncillaryNode ancillary}
     * @param cons Consumer to execute for each ancillary
     */
    ...
    public @Nullable AncillaryNode<?> getAncillary() {
        ...
        for(int x = 0; x < ancillaries.length; x++) {
            ...
            // prioritize the first wireancillary encountered
