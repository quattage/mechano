...
public class EntropyTracker {
    ...
    // TODO this class is LOD unfriendly and a little expensive to store in every single catenary instance
    ...
    public boolean isResting() {
        if(avgVelocity < Catenaries.renderPipeline().getRestitutionSpeed() && (Math.abs(accumulatedError - previousAccumulatedError) < Catenaries.renderPipeline().getRestitutionEpsilon())) {
    ...
    public void walk(float newError, int steps) {
        previousAccumulatedError = accumulatedError;
        accumulatedError = (newError / (float)(steps * Catenaries.renderPipeline().getSolverSteps()));
