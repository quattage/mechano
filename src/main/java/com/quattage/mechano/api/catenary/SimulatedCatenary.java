...
import com.quattage.mechano.api.catenary.meshing.CatenaryRenderFeatures.Point;
import com.quattage.mechano.api.catenary.meshing.CatenaryRenderFeatures.Stick;
import com.quattage.mechano.api.catenary.meshing.EntropyTracker;
import com.quattage.mechano.api.catenary.meshing.MeshExtruder;
...
public class SimulatedCatenary extends CatenaryModel<SimulatedCatenary> {
    ...
    public SimulatedCatenary setOrderedOffset(LevelReader world, TransmitterType<?> trns, @Nullable GridUUID start, @Nullable GridUUID end, float pTicks) {
        if(start == null || end == null) return this;
        Duo<GridUUID> ordered = TrackedObject.orderedByAssertionPriority(world, start, end);
    ...
    public SimulatedCatenary setOrderedOffset(LevelReader world, TransmitterType<?> trns, @Nullable AnchorPoint start, @Nullable AnchorPoint end, float pTicks) {
        if(start == null || end == null) return this;
        Duo<AnchorPoint> ordered = TrackedObject.orderedByAssertionPriority(world, start, end);
        setOffset(trns, ordered.first().getPos(world, pTicks), ordered.second().getPos(world, pTicks));
        return this;
    }
