...
public class BatteryDatasheet implements Cloneable {
    ...
    private final Bifrucated64 ratedCapacity;      // amp hours
    private final float internalResistance;    // ohms
    private final float chargeEfficiency;   // [0, 1] scalar
    ...
    public static final BatteryDatasheet DEFAULT = new BatteryDatasheet(
        new Bifrucated64(40), 0.015f, 0.98f, 0.9f, 0.9f, // c rates are exaggerated because car batteries suck irl
    ...
    public BatteryDatasheet(Bifrucated64 ratedCapacity, float internalResistance, float dischargeEfficiency, float chargeRate, float dischargeRate, VoltageDecay voltage) {
        ...
        this.chargeEfficiency = dischargeEfficiency;
    ...
    public boolean isWithinCapacity(Bifrucated64 charge) {
    ...
    /**
     * The {@link VoltageDecay } of this datasheet maps <code>SoC</code> to <code>V_oc</code>
     * In other words, it describes the voltage response of a battery given how full it is.
     * @return A VoltageDecay LUT
     */
    public VoltageDecay getVoltageLUT() { return voltage; }
    public float getInternalResistance() { return internalResistance; }
    public float getChargeEfficiency() { return chargeEfficiency; }
     ...
     */
    public boolean isLossy() { return chargeEfficiency < 0.9999; }
    ...
    public BatteryDatasheet copyWith(VoltageDecay voltage) {
        return new BatteryDatasheet(ratedCapacity, internalResistance, chargeEfficiency, chargeRate, dischargeRate, voltage);
    }
    ...
    public BatteryDatasheet copyWith(Function<DataPoint[], VoltageDecay> func) {
        return new BatteryDatasheet(ratedCapacity, internalResistance, chargeEfficiency, chargeRate, dischargeRate, func.apply(this.voltage.getTable()));
    }
}
