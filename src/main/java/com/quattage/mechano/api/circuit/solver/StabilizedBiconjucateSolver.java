...
public class StabilizedBiconjucateSolver implements NodalSolver {
    ...
    private double rho, rho_old, alpha, omega, d, normS, ts, tt, normR, beta;
    ...
    public ConvergenceStatus run(NodalSnapshot snapshot) {
        ...
        for(int i = 0; i < NodalSolver.STEP_LIMIT; i++) {
            ...
            ts = VectorVectorMult_DDRM.innerProd(t, s);
            tt = VectorVectorMult_DDRM.innerProd(t, t);
            omega = ts / tt;
    ...
    public int estimateMemoryFootprint(NodalSnapshot snapshot) {
        return NodalSolver.estimateMemoryFootprint(8, 10, 0, snapshot.termA().getNumRows());
    }

    @Override
    public void reset() {
        rho = 0; rho_old = 0; alpha = 0; omega = 0; d = 0; normS = 0; ts = 0; tt = 0; normR = 0; beta = 0;
