 ...
 * the {@link com.quattage.mechano.api.ServerMatrix ServerMatrix}. 
...
public interface Transmitable<T extends Transmitter<?>> {
     ...
     */
    default GridResponse collectTooltipInfoAndResponse(ClientLevel world, List<Component> tooltip, Griddable points, AnchorPoint target, HoldingSummary held) {
        Mechano.lang().text("hi >:)").forGoggles(tooltip);
     ...
     */
    default boolean onRenderTick(LevelReader world, HoldingSummary held, DeltaTracker delta) {
        return true;
    }

    TransmitterType<T> getTransmitterType();
    ...
    static HoldingSummary getHolding(Player player) {
