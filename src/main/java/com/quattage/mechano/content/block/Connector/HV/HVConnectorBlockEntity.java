...
public class HVConnectorBlockEntity extends BaseElectricTileEntity implements IWireNode, IObserveTileEntity, IHaveGoggleInformation {
    ...
    public HVConnectorBlockEntity(BlockEntityType<?> tileEntityTypeIn, BlockPos pos, BlockState state) {
	...
	public Vec3d getNodeOffset(int node) {
		switch(getCachedState().get(HVConnectorBlock.FACING)) {
			case DOWN:
				return OFFSET_DOWN;
			case UP:
				return OFFSET_UP;
	...
	public boolean isEnergyInput(Direction side) {
		return getCachedState().get(HVConnectorBlock.FACING) == side;
	}
	...
	private void networkTick(EnergyNetwork en) {
		if(world.isClient())
			return;
		Direction d = getCachedState().get(HVConnectorBlock.FACING);
