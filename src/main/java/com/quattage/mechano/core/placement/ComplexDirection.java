...
public enum ComplexDirection implements StringRepresentable {
    ...
    private static final Int2ObjectMap<ComplexDirection> COMBINED_LOOKUP = Util.make(new Int2ObjectOpenHashMap<>(values().length), (boysmell) -> {
        for(ComplexDirection direction : values()) {
            boysmell.put(lookupKey(direction.cardinal, direction.orient), direction);
        }
    });

    private ComplexDirection(String name, Direction cardinal, Axis orient) {
        this.name = name;
    ...
    public static ComplexDirection combine(Direction cardinal, Axis orient) {
        if(cardinal == null) 
    ...
    public static ComplexDirection cycleOrient(ComplexDirection in) {
        int pos = in.ordinal();
        if(pos % 2 == 0) pos += 1;
        else pos -= 1;
        return ComplexDirection.values()[pos];
    }

    public static ComplexDirection cycle(ComplexDirection in) {
