...
public enum VerticallyJustifiedComplexDirection implements StringRepresentable {
    // stores t
    ...
    private static final Int2ObjectMap<VerticallyJustifiedComplexDirection> COMBINED_LOOKUP = Util.make(new Int2ObjectOpenHashMap<>(values().length), (boysmell) -> {
        for(VerticallyJustifiedComplexDirection direction : values()) {
            boysmell.put(lookupKey(direction.localFacing, direction.localVertical), direction);
        }
    });

    private VerticallyJustifiedComplexDirection(String name, Direction localFacing, Direction localVertical) {
        this.name = name;
    ...
    public static VerticallyJustifiedComplexDirection combine(Direction localFacing, Direction localVertical) {
        if(localFacing == null) 
    ...
    public static VerticallyJustifiedComplexDirection flipVertical(VerticallyJustifiedComplexDirection in) {
        int pos = in.ordinal();
        if(pos % 2 == 0) pos += 1;
        else pos -= 1;
        return VerticallyJustifiedComplexDirection.values()[pos];
    }

    public static VerticallyJustifiedComplexDirection cycle(VerticallyJustifiedComplexDirection in) {
        int pos = in.ordinal();
        pos = pos += 2;
        if(pos > 7) pos -= 7;
        return VerticallyJustifiedComplexDirection.values()[pos];
