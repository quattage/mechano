...
import com.quattage.mechano.foundation.api.catenary.RestitutionTracker;
    ...
    */
    ...
    public void update() {
        ...
        if(WindManager.INSTANCE.isEnabled() && wind != null) 
            integrateVelocity(workingVector, gravity, wind);
        else integrateVelocity(workingVector, gravity);

        if(tracker.isCascading()) {
        ...
        tracker.walk(applyConstriants(workingVector), points.size());
    }
     ...
     * @param vec Working vector passed here to avoid continuous re-declaration
     * @return The total length of each segment in this catenary 
     */
    public float applyConstriants(Vector3f vec) {
        float error = 0;
