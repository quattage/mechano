...
public final class ClientGrid extends Grid<GridUUID<?>> {
    ...
    @Override protected void read(LevelReader world, CompoundTag contents, Provider provider) {}
    @Override protected void write(LevelReader world, CompoundTag contents, Provider provider) {}
    ...
    @Override
    public GridAction removeLink(GridUUID<?> a, GridUUID<?> b, @Nullable Entity modifier) {
        super.removeLinkAsymmetric(a.copyAndClearBindings(), b);
        return super.removeLinkAsymmetric(b.copyAndClearBindings(), a);
    }
    ...
    public static class LinkDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
        ...
        public void render(PoseStack matrixStack, MultiBufferSource bufferSource, double camX, double camY, double camZ) {
            if(!enabled) return;
            ClientGrid grid = Grid.client(minecraft.level);
