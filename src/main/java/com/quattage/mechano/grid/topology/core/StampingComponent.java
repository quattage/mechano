...
public abstract class StampingComponent extends DiscreteComponent implements TerminalProvider {
    ...
    // public int inferIndexOfSelf(ServerGrid grid) {
    //     Objects.requireNonNull(grid);
    //     for(int x = 0; x < terminals.length; x++) {
    //         Node node = terminals[x].getAttachedNode();
    //         if(node == null) throw new NullPointerException("Failed while getting index of node at " + terminals[x] + " - This terminal's attached node is null!");
     ...
     */
    public double voltageOf(Netlist netlist, Terminal terminal) {
        Objects.requireNonNull(netlist);
        Objects.requireNonNull(terminal);
        Node node = terminal.getAttachedNode();
        return voltageOf(netlist, node);
    }
     ...
     * @return double value representing voltage 
     */
    public double voltageOf(Netlist netlist, @Nullable Node node) {
        Objects.requireNonNull(netlist);
        if(node == null) return 0;
        return 1;
        // return netlist.indexer().get(node); TODO FIX
    }
     ...
     */
    public abstract void stamp(ServerGrid grid, Netlist netlist);
    ...
    public interface StampsDynamically {
         ...
         * @see #stamp
         */
        void stampDynamic(ServerGrid grid, Netlist netlist);
    }
    ...
    public interface NeedsPostProcessing {
         ...
         * @param netlist The netlist this component belongs to
         * @param current The current (in amps) that exists at this component
         */
        void postProcess(ServerGrid grid, Netlist netlist);
    }
    ...
    public interface CurrentChangeCallback {
         ...
         * @param grid Domain to operate within
         ...
         */
        void onCurrentUpdated(Netlist netlist, PowerState then, PowerState now);
    }
    ...
    public static class PowerState {
        ...
        public double joules() {
            return watts() * SolverAlgorithm.DELTA;
        }
        ...
        public double ampHours() {
            return amps() * SolverAlgorithm.DELTA / 3600d;
        }

        public boolean isCurrentEqual(PowerState other) {
            return Math.abs(other.amps() - this.amps()) < SolverAlgorithm.EPSILON;
        }

        public boolean isVoltageEqual(PowerState other) {
            return Math.abs(other.volts() - this.volts()) < SolverAlgorithm.EPSILON;
        }
