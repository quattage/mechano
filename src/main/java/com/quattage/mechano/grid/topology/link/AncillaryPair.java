...
public class AncillaryPair extends NodePair implements CircuitComponent {
    ...
    public void MNAAllocate(GridDomain domain) {
        assertHasNodes();
        getStartAncillary().MNAAllocate(domain);
        getEndAncillary().MNAAllocate(domain);
    }
    ...
    public void MNADeallocate(GridDomain domain) {
        ...
        if(!domain.netlist().hasConnections(getStartNode()))
            getStartAncillary().MNADeallocate(domain);
        if(!domain.netlist().hasConnections(getEndNode()))
            getEndAncillary().MNADeallocate(domain);
    }
    ...
    public AncillaryPair getFlipped(LevelReader world) {
        return Grid.client(world).lookup().getLinkMatching(endID, startID);
    }
